...
for (i in 1:nrow(select_data)) {
  poach.level <- select_data$poach.density[i]
   if (select_data$poach.density[i] <= 0.005) {
     poach.level[i] <- "miniscule"
   } else if (select_data$poach.density[i] <= 0.01) {
     poach.level[i] <- "low"
   } else if (select_data$poach.density[i] <= 0.015) {
     poach.level[i] <- "medium"
   } else if (select_data$poach.density[i] <= 0.02) {
     poach.level[i] <- "high"
   } else {
...
Calculate mean poaching per km in each year
